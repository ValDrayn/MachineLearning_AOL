...
def copy_data_to_yolov11(repo_path, yolov11_path):
    src_data_path = os.path.join(repo_path, 'data')
    dst_data_path = os.path.join(yolov11_path, 'data')
    ...
    update_yaml(os.path.join(yolov11_path, 'dataset.yaml'), dst_data_path)

def main(opt):
    repo_path = opt.reporoot
    yolov11_path = os.path.join(repo_path, 'yolov11')
